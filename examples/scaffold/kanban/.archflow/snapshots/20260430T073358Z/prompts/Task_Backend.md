...
- [Task_API] from @Client_UI
  - Schema: Client sends task create/list/update/delete requests as JSON over HTTP.
...
## Internal Workflows
- $Task_API_Flow: state:Idle >> action:Receive_Task_Request >> action:Validate_Input >> action:Persist_Task >> action:Return_Response >> state:Request_Handled >> state:Idle >> action:Receive_Delete_Request >> action:Validate_Task_Id >> action:Delete_Task >> action:Return_Delete_Response >> state:Request_Handled
